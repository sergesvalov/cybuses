...
<head>
    ...
    <style>
        ...
        /* Если есть заметка, делаем кнопку кликабельной на вид */
...
<body>
    <div class="header">
        <h2 style="margin:0">🇨🇾 Bus Board</h2>
        <button class="refresh-btn" id="r-btn" onclick="refresh()">↻</button>
    </div>
    ...
    <script>
        ...
        function render() {
            ...
            fData.forEach(r => {
                ...
                // HTML for Buttons
                // Проверяем, есть ли текст заметки (note_txt) или просто звездочка
                const timesHtml = r.times.map(t => {
                    const stars = t.n || '';
                    const noteTxt = t.note_txt ? t.note_txt.replace(/"/g, '&quot;') : ''; 
                    const hasNoteClass = noteTxt ? 'has-note' : '';
                    
                    // Если есть заметка, добавляем onclick с текстом
                    // Если нет, кнопка не кликабельна
                    const action = noteTxt 
                        ? `onclick="showN('${t.t}', '${stars}', '${noteTxt}')"` 
                        : '';
        ...
        // n - это теперь полный текст сноски
